...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ media, isOpen, onClose, startTime = 0, onPlayNext }) => {
  ...
  const getStreamUrl = (mediaId: number, quality?: string, format?: string, seekTime?: number) => {
    ...
    // Add seek time for transcoded streams
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      progressInterval = setInterval(() => {
        const video = videoRef.current;
        if (video && !video.paused && video.currentTime > 30) {
          const currentDuration = duration || (video.seekable && video.seekable.length > 0 ? video.seekable.end(video.seekable.length - 1) : 0);
          ...
          onLoadedMetadata={async () => {
            ...
            try {
              ...
              if (resumeTimeValue > 30 && videoDuration > 60 && resumeTimeValue < videoDuration - 30) {
                ...
                console.log('Resume progress tracked:', resumeTimeValue, 'of', videoDuration);
              } else if (startTime > 0) {
