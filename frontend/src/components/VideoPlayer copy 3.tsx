...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ media, isOpen, onClose, startTime = 0, onPlayNext }) => {
  ...
  useEffect(() => {
    if (isOpen && media.id && !videoSrc) {
      ...
      if (video) {
        ...
        console.log('🎬 Setting video source (ONCE):', initialVideoSrc);
  ...
  const saveCurrentProgress = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ Progress saved:', video.currentTime, 'of', currentDuration);
    } catch (error) {
  ...
  const togglePlay = useCallback(async () => {
    try {
      ...
      if (video.paused) {
        console.log('▶️ Resuming video from current position:', video.currentTime);
        // Just resume playback from current position - no source changes
        video.play().catch((error) => {
          console.error('Failed to resume video:', error);
        });
      } else {
        console.log('⏸️ Pausing video at current position:', video.currentTime);
        ...
        >
          ...
          <video
            ...
            onPause={async () => {
              ...
              console.log('🔴 Saving progress on pause...');
              await saveCurrentProgress();
            }}
            ...
            onClick={async (e) => {
              ...
              if (videoRef.current) {
                ...
                if (video.paused) {
                  console.log('🖱️ Video clicked - resuming from:', video.currentTime);
                  // Just resume playback, don't reload or make new requests
                  video.play().catch(() => {
                    // Play failed on video click
                  });
                } else {
                  console.log('🖱️ Video clicked - pausing at:', video.currentTime);
                  // Save progress before pausing
                  await saveCurrentProgress();
            ...
            onLoadStart={() => {
              console.log('🔄 Video load started - this should only happen on initial load!');
              setIsLoading(true);
              setIsBuffering(true);
            ...
            onLoadedMetadata={async () => {
              ...
              if (!hasInitiallyLoaded) {
                console.log('🔄 Loading progress on initial metadata load');
                try {
                  ...
                  console.log('📊 Progress data:', { startTime, savedPosition: savedProgress?.position, resumeTimeValue });

                  // Only auto-seek on initial load when video is at beginning
                  if (video.currentTime < 5) {
                    if (startTime > 0) {
                      // Use provided start time
                      console.log('⏭️ Seeking to provided start time:', startTime);
                      ...
                      console.log('⏭️ Auto-resuming from saved position:', resumeTimeValue);
                      video.currentTime = resumeTimeValue;
                      setCurrentTime(resumeTimeValue);
                      setResumeTime(resumeTimeValue);
                    } else {
                      console.log('▶️ Starting from beginning (no valid resume position)');
                    }
                  } else {
                    console.log('⚠️ Video already positioned at:', video.currentTime, '- not seeking');
                  }
                } catch (error) {
                ...
                console.log('⚠️ Metadata loaded but already initialized - ignoring');
              }
            }}
            onLoadedData={() => {
              setIsLoading(false);
      ...
      {showPauseScreen && !isLoading && !isBuffering && !isCasting && (
        <motion.div
          ...
          onClick={(e) => {
            ...
            const video = videoRef.current;
            if (video && video.paused) {
              console.log('🖱️ Pause screen clicked - resuming from:', video.currentTime);
