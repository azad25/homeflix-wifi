...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ media, isOpen, onClose, startTime = 0, forceStartFromBeginning = false, onPlayNext, onVideoPlay, onProgress }) => {
  ...
  const setupAudioEnhancement = useCallback(() => {
    ...
    try {
      ...
      // Create source node if it doesn't exist (or reconnect if it was in bypass mode)
      ...
      if (!compressorNodeRef.current) {
        ...
        // Initialize with bypass settings - will be updated by updateAudioEnhancement
        compressorNodeRef.current.threshold.value = 0;
        compressorNodeRef.current.knee.value = 0;
        compressorNodeRef.current.ratio.value = 1;
        compressorNodeRef.current.attack.value = 0.001;
        compressorNodeRef.current.release.value = 0.15;
      }
      ...
      // Apply current settings after a small delay to ensure state is updated
      setTimeout(() => {
  ...
  const updateAudioEnhancement = useCallback(() => {
    ...
    try {
      ...
      if (compressorNodeRef.current) {
        // Compressor parameters use .value for immediate effect
      ...
      // Update gain and bass boost (volume boost) - these use smooth ramping
      if (gainNodeRef.current && bassBoostNodeRef.current && midBoostNodeRef.current) {
        if (volumeBoostEnabled) {
          gainNodeRef.current.gain.setTargetAtTime(volumeBoostLevel, currentTime, rampTime);
          const bassGain = Math.min((volumeBoostLevel - 1) * 5, 10);
          bassBoostNodeRef.current.gain.setTargetAtTime(bassGain, currentTime, rampTime);
          const midGain = Math.min((volumeBoostLevel - 1) * 1.5, 3);
          midBoostNodeRef.current.gain.setTargetAtTime(midGain, currentTime, rampTime);
        } else {
          gainNodeRef.current.gain.setTargetAtTime(1.0, currentTime, rampTime);
          bassBoostNodeRef.current.gain.setTargetAtTime(0, currentTime, rampTime);
          midBoostNodeRef.current.gain.setTargetAtTime(0, currentTime, rampTime);
        }
      }
  ...
  // Apply audio settings with explicit values (for immediate updates)
  const applyAudioSettings = useCallback((stableVolume: boolean, volumeBoost: boolean, boostLevel: number) => {
    ...
    try {
      ...
      // Reconnect audio chain to bypass or include compressor
      const source = sourceNodeRef.current;
      const compressor = compressorNodeRef.current;
      const bassBoost = bassBoostNodeRef.current;
      const midBoost = midBoostNodeRef.current;
      ...
      if (source && compressor && bassBoost && midBoost && gain && limiter) {
      ...
      if (gainNodeRef.current && bassBoostNodeRef.current && midBoostNodeRef.current) {
        if (volumeBoost) {
          gainNodeRef.current.gain.setTargetAtTime(boostLevel, currentTime, rampTime);
          const bassGain = Math.min((boostLevel - 1) * 5, 10);
          bassBoostNodeRef.current.gain.setTargetAtTime(bassGain, currentTime, rampTime);
          const midGain = Math.min((boostLevel - 1) * 1.5, 3);
          midBoostNodeRef.current.gain.setTargetAtTime(midGain, currentTime, rampTime);
        } else {
          gainNodeRef.current.gain.setTargetAtTime(1.0, currentTime, rampTime);
          bassBoostNodeRef.current.gain.setTargetAtTime(0, currentTime, rampTime);
  ...
  const cleanupAudioEnhancement = useCallback(() => {
    try {
      ...
      setTimeout(() => {
        try {
          // Disconnect the processing chain
          if (limiterNodeRef.current) {
  ...
  const handleStableVolumeChange = useCallback((enabled: boolean) => {
    ...
    if (enabled) {
      ...
      if (!audioEnhancementActiveRef.current) {
        ...
        applyAudioSettings(enabled, volumeBoostEnabled, volumeBoostLevel);
      }
    } else {
      // Cleanup when disabling and no other features are active
      if (!volumeBoostEnabled) {
  ...
  }, [volumeBoostEnabled, volumeBoostLevel, setupAudioEnhancement, cleanupAudioEnhancement]);
  ...
  const handleVolumeBoostChange = useCallback((enabled: boolean, level: number) => {
    ...
    if (enabled) {
      ...
      if (!audioEnhancementActiveRef.current) {
        ...
        applyAudioSettings(stableVolumeEnabled, enabled, level);
      }
    } else {
      // Cleanup when disabling and no other features are active
      if (!stableVolumeEnabled) {
  ...
  }, [stableVolumeEnabled, setupAudioEnhancement, cleanupAudioEnhancement]);
      ...
      {isOpen && (
        ...
        >
          ...
          <video
            ...
            poster={(() => {
              // For episodes, prioritize episode still, then series backdrop
              if (media.type === 'episode') {
                if (media.episode_still_path) return `${getApiUrl()}/api/episode-stills/${media.id}`;
                if (seriesData?.backdrop_path) return `${getApiUrl()}/api/${seriesData.backdrop_path}`;
                if (seriesData?.tmdb_backdrop_url) return seriesData.tmdb_backdrop_url;
              }
            ...
            {showControls && (
              ...
              >
                  ...
                  <div>
                    {(() => {
                      ...
                      return logoUrl ? (
                        ...
                        <h1 className="text-white text-2xl font-bold">{media.episode_title || media.title}</h1>
                      );
                    })()}
                  </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-10 pointer-events-auto z-50">
                  <div
                    className="progress-container relative w-full h-1 cursor-pointer hover:h-1.5 transition-all duration-150 mb-4 group"
                    onClick={(e) => {
                  ...
                  >
                    ...
                    <div className="absolute inset-0 w-full h-full bg-white/30 rounded-lg overflow-hidden pointer-events-none group-hover:bg-white/40 transition-colors duration-150">
                      {/* Buffered Progress */}
                      <div
                        className="absolute top-0 left-0 h-full bg-white/20 group-hover:bg-white/30 transition-colors duration-150"
                        style={{
                    ...
                    {/* Progress Handle */}
                    <div
                      className={`absolute top-1/2 transform -translate-y-1/2 -translate-x-1/2 w-3 h-3 bg-red-600 rounded-full transition-all duration-200 pointer-events-none ${isDragging || isBuffering ? 'opacity-100 scale-150' : 'opacity-0 group-hover:opacity-100 group-hover:scale-125'
                        }`}
                      style={{
                        left: `${(() => {
                          if (isCasting && castState.isConnected) {
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {media.type === 'episode' && (media.episode_title || media.title) && (
                      <motion.div
                        initial={{ y: 20, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        exit={{ y: 20, opacity: 0 }}
                    ...
                    {/* Episode Description - prioritize episode description over series */}
                    {media.type === 'episode' && (media.description || media.long_desc || media.short_desc) ? (
                      <motion.p
                        initial={{ y: 20, opacity: 0 }}
                        animate={{ y: 0, opacity: 1 }}
                        exit={{ y: 20, opacity: 0 }}
                      ...
                      >
                        {media.description || media.long_desc || media.short_desc}
                      </motion.p>
