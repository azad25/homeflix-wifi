...
func NewTorrentClient(downloadDir string, mediaScanner MediaScannerInterface, statusCallback StatusCallback, performanceConfig ...map[string]int) (*TorrentClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to normalize download directory: %v", err)
	}
...
func normalizeDownloadDir(downloadDir string) (string, error) {
...
func (tc *TorrentClient) findExistingSessionTorrent(torrentID, magnetURI string) *raintorrent.Torrent {
	if torrentID != "" {
		if existing := tc.session.GetTorrent(torrentID); existing != nil {
			return existing
		}
	}

	targetHash := extractInfoHashFromMagnet(magnetURI)
	if targetHash == "" {
		return nil
	}

	for _, existing := range tc.session.ListTorrents() {
		if existing == nil {
			continue
		}
		existingMagnet, err := existing.Magnet()
		if err != nil || existingMagnet == "" {
			continue
		}
		existingHash := extractInfoHashFromMagnet(existingMagnet)
...
func (tc *TorrentClient) RestoreDownload(torrentID, name, magnetURI, status, savePath string, progress float64, size, downloaded int64, addedAt time.Time, completedAt *time.Time) error {
	...
	var t *raintorrent.Torrent
	existing := tc.findExistingSessionTorrent(torrentID, magnetURI)
	if existing != nil {
		t = existing
	} else {
		created, err := tc.session.AddURI(magnetURI, nil)
		if err != nil {
	...
	if status == "downloading" {
		startErr := t.Start()
		if startErr != nil {
			log.Printf("⚠️ Failed to start restored torrent: %v", startErr)
